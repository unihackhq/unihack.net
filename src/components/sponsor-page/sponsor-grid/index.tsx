...
export const SponsorGrid = ({prefix, items, id, className, gridItemClassName}: SponsorGridProps) => {
  return (
      <div id={id} className={styles.sponsorGridWrapper}>
        ...
        <div className={mergeClassNames(styles.sponsorGrid, className)}>
          {items.map((({link,...item}, key) => {
            return (<a key={key} className={mergeClassNames(styles.sponsorGridItem, gridItemClassName)} href={link}>
              <Image {...item} />
            </a>)
          }))}
        </div>
